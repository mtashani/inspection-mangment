...
# Create a dedicated router for service risk that's not under /api/psv
...
def update_service_risk_category(
    ...
    """Update existing service risk category by ID"""
    category = db.get(ServiceRiskCategory, id)
    
    if not category:
        raise HTTPException(
            status_code=404,
            detail=f"No risk category found with id: {id}"
        )
...
def delete_service_risk_category(
    ...
    return {"message": f"Service risk category with id {id} deleted successfully"}